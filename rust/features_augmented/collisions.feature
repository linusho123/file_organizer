Feature: Name collision handling
  A file being moved that has the same name as a file already in the
  destination type folder is renamed with the lowest available numeric
  suffix. Nothing is ever overwritten or lost, and every rename is
  reported in the Issues section. (PRD FR-9..FR-11)
